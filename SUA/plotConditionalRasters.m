function plotConditionalRasters(rAxesH, pAxesH, spikeTs,eventTs, conditionIdxs, conditionLabels, windowMs, binSizesMs)
%PLOTCONDITIONALRASTERS Summary of this function goes here
...
binsEarlierTimeS            = -(windowS/2) + (-(windowS/2) *0.3);
binsLaterTimeS              = (windowS/2) + ((windowS/2)*0.3);
...
lineStyleStr                = 'vertline';
rasterLineWidth             = 4.5;%1.5
group_colors                = lines(numConditions);
...
conditionSpikeTs = cell(numConditions,1);
conditionMeanY = nan(numConditions,1);
current_y = 0;
...
for c = 1:numConditions
    ...
    for e = 1:numel(cEventTs)
        ...
        eventWindowStartS = (eventTimeS - (windowS/2));
        
        eventWindowStopS = (eventTimeS + (windowS/2));
        
        
        windowSpikeRelS = spikeTs(find(spikeTs >= eventWindowStartS & spikeTs <= eventWindowStopS)) - (eventTimeS);
    ...
    line_format = struct();
    line_format.Color = group_colors(c, :);
    line_format.LineWidth = rasterLineWidth;
    line_format.LineStyle = '-';
    
    
    %% Plot Raster on rAxesH
    axes(rAxesH);
    hold on;
    plotSpikeRaster(eventSpikeS, 'PlotType', lineStyleStr, 'LineFormat', line_format, 'VertSpikePosition', current_y);
    
    
    conditionSpikeTs{c} = eventSpikeS;
    
    
    conditionMeanY(c) = mean([current_y, (current_y+numel(cEventTs))]);
    
    
    current_y = current_y+numel(cEventTs);
    
    ylim([0 current_y]);
    xlim([-(windowS/2) (windowS/2)]);
    ...
    xlim([-(windowS/2) (windowS/2)]);
    mean_eventBinnedHz = nanmean(eventBinnedHz);
    eventBinnedHz_var = std(eventBinnedHz)/sqrt(size(eventBinnedHz,1));
    
    eventBinnedHz_upperCI =mean_eventBinnedHz+eventBinnedHz_var;
    eventBinnedHz_lowerCI =mean_eventBinnedHz-eventBinnedHz_var;
    
    patchX = horzcat(binMeanS, fliplr(binMeanS));
    patchY =  horzcat(eventBinnedHz_upperCI, fliplr(eventBinnedHz_lowerCI));
    
    
    
    group_patch_H(c) = patch('XData', patchX,'YData', patchY,'FaceColor', group_colors(c, :), 'FaceAlpha',.2, 'EdgeColor', 'none');
    plot(binMeanS, mean_eventBinnedHz, '--','Color', group_colors(c, :), 'LineWidth', .75);
    
    
end

    axes(rAxesH);
    yticks(conditionMeanY);
    ...
    axes(pAxesH);
    vline(0, 'r:');
    legend(group_patch_H,conditionLabels)
     xlabel('Time (s)');
